...
interface IngredientDao {
     ...
     */
    @get:Query("SELECT * FROM ingredient WHERE ingredient_selected = 1 AND ingredient_excluded = 0;")
     ...
     */
    @get:Query("SELECT *  FROM ingredient GROUP By ingredient_category;")
    val allCategories: List<Ingredient>

    /**
     * Get all of the ingredient where they are not excluded (an intolerance)
     */
    @get:Query("SELECT *  FROM ingredient WHERE ingredient_excluded = 0")
    val allTolerantIngredient: List<Ingredient>
    ...
    fun getIngredientsByCategory(ingredientCategory: String): List<Ingredient>
    ...
    fun getIngredientsById(ingredientIds: List<Int>): List<Ingredient>
